...
export default class CharacterModal extends React.Component {
  ...
  openModal() {
    ...
    Promise.all(data.species.map(e => this.fetchData(e)))
      ...
      .finally(
        this.state.species.length === 0 && this.setState({ species: null })
      );
    ...
    Promise.resolve(this.fetchData(data.homeworld))
      ...
      .finally(
        this.state.homeworld.length === 0 && this.setState({ homeworld: null })
      );
    ...
    Promise.all(data.vehicles.map(e => this.fetchData(e)))
      ...
      .finally(
        this.state.vehicles.length === 0 && this.setState({ vehicles: null })
      );

    Promise.all(data.starships.map(e => this.fetchData(e)))
      .then(response =>
        response.map(starships => {
      ...
      .finally(
        this.state.starships.length === 0 && this.setState({ starships: null })
      );
        ...
        >
          ...
          <div className="modal">
            ...
            <div className="transpose-table">
              <Table data={this.state.general} title={this.props.item.name} />
            </div>
            <div className="transpose-table">
              <Table data={this.state.species} title={"Species"} />
            </div>
            <div className="transpose-table">
              <Table data={this.state.homeworld} title={"Homeworld"} />
            </div>
            <Table data={this.state.vehicles} title={"vehicles"} />
            <Table data={this.state.starships} title={"starships"} />
            <Table data={this.state.films} title={"Films"} />
          </div>
        </Modal>
      </div>
    );
